  ...
  return (
    ...
    >
      <div className="ml-auto flex h-full w-full max-w-2xl flex-col overflow-hidden bg-white shadow-xl">
        <header className="flex items-center justify-between border-b border-[color:var(--border)] bg-red-50 p-4">
          <div>
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-4 text-sm">
          ...
          <section>
            <p className="mb-1 text-xs font-medium uppercase tracking-wider text-text-3">
              old_value
            </p>
...
function Row({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="grid grid-cols-[160px_1fr] gap-3 border-b border-[color:var(--border)] pb-2">
...
function JsonBlock({ value }: { value: unknown }) {
  if (value == null)
    return <span className="text-xs text-text-3">null</span>
  return (
    <pre className="max-h-80 overflow-x-auto overflow-y-auto rounded bg-navy-dark p-3 text-xs text-white/90">
