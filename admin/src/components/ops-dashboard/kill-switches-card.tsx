    ...
    <section className="rounded-md border border-[color:var(--border)] bg-white shadow-sm">
      ...
      <div className="flex flex-col gap-2 p-4">
        {switches.map((sw) => (
          <div
            key={sw.switch_key}
            className="flex items-center justify-between rounded border border-[color:var(--border)] p-2"
          >
            <div>
              <div className="font-mono text-sm">{sw.switch_key}</div>
              <div className="text-xs text-text-3">{sw.description}</div>
        ...
        <Link
          ...
          className="mt-2 block rounded border border-[color:var(--border-mid)] bg-white px-3 py-2 text-center text-xs text-text-2 hover:bg-bg"
        >
