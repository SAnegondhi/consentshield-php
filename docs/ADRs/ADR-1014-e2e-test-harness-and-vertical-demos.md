...
- [x] DNS cutover complete: **`demo-healthcare.consentshield.in`** — custom domain provisioned on the Railway service (verified via GraphQL `customDomains` + live `curl`). **Naming drift:** the fixture `allowed_origins` in `scripts/e2e-bootstrap.ts` still lists `demo-clinic.consentshield.in`; the Cloudflare CNAME landed on `demo-healthcare.consentshield.in`. Follow-up to reconcile: either rename the Cloudflare record to `demo-clinic`, or update the fixture + ADR-1014's stated target to `demo-healthcare`. Until reconciled, the live custom domain will fail the Worker's origin check for consent POSTs. Flagged as a one-line fixture edit.
