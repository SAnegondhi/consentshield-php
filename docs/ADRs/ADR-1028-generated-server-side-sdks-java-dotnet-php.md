...
- [x] Speakeasy bake-off review at `docs/reviews/2026-04-26-tier2-generator-evaluation.md` — placeholder structure landed; Speakeasy run captured by operator (sign-up + CLI invocation), file updated with side-by-side diff before Sprint 1.2 starts.
...
**Generator choice for v1.0.0: OpenAPI Generator (`openapitools/openapi-generator-cli:v7.10.0`).** Free, open-source, image tag exact-pinned per Rule 17. Speakeasy / Stainless / Fern explicitly deferred — their polish is real but the cost ($1k–$5k/month + spec-in-their-pipeline) is not justified pre-revenue. **Re-evaluation trigger:** MRR > $50k OR first BFSI Tier-1 enterprise prospect explicitly comments on generated-client code quality. The decision is reversible — the input is the same OpenAPI spec, swapping tools is a one-CI-step change.
