...
| ADR-1005 | v2 Whitepaper Phase 5 — Operations maturity (webhook reference, test_delete, support model, status page, rights API, non-email channels); Sprint 5.1 shipped 2026-04-22 (`POST` / `GET /v1/rights/requests` with identity attestation + captured_via + created_via_api audit event); Phase 6 Sprint 6.1 shipped 2026-04-22 (NotificationAdapter interface + retry helper + registry + mock + dispatcher; 20/20 unit tests PASS); Phase 2 Sprint 2.1 shipped 2026-04-22 (`POST /v1/integrations/{connector_id}/test_delete` + `rpc_test_delete_trigger`; cs_api surface 22 → 23 RPCs); **Phase 6 Sprints 6.2 + 6.3 shipped 2026-04-23** (Slack + Teams + Discord + PagerDuty + custom_webhook adapters; 51 new unit tests + live Slack delivery verified end-to-end; Teams/Discord/PagerDuty live tests deferred per channel-account decisions tracked on admin.ops_readiness_flags); Sprint 6.4 (dashboard UI + alert routing matrix) remains | In Progress | 2026-04-19 | 6 | 10 |
