...
| ADR-1018 | Self-hosted status page (supersedes ADR-1005 Phase 4 StatusPage.io plan). `public.status_subsystems` + `status_checks` + `status_incidents` with anon SELECT + admin-RPC writes; admin panel at `/admin/(operator)/status` (state flips + post/update/resolve incidents); public read-only page at `app.consentshield.in/status`. Sprints 1.1 + 1.2 + 1.3 shipped 2026-04-22; probe-cron Sprint 1.4 + DNS-alias Sprint 1.5 pending. Setup runbook at `docs/runbooks/status-page-setup.md`. | In Progress | 2026-04-22 | 1 | 5 |
