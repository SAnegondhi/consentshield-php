-- Migration: ADR-0053 Sprint 1.1 follow-up — fix nested-aggregate error in
-- admin.billing_gstr1_json. Postgres forbids `jsonb_agg(jsonb_build_object(
-- 'inv', jsonb_agg(...)))`. Rewrite B2B + B2CL as two-step CTEs: first build
-- per-invoice rows, then aggregate into per-counterparty/per-pos groups.
...
begin
  ...
  -- ══════════════════════════════════════════════════════════════════════
  -- B2B: per-invoice rows then aggregated per ctin
  -- ══════════════════════════════════════════════════════════════════════
  with b2b_rows as (
    select
   ...
   where i.issuer_entity_id = p_issuer_id
    ...
    select ctin, jsonb_agg(inv_row) as inv_array
      from b2b_rows
     group by ctin
  )
  ...
  -- ══════════════════════════════════════════════════════════════════════
  -- B2CL: per-invoice rows then aggregated per pos
  -- ══════════════════════════════════════════════════════════════════════
  with b2cl_rows as (
    select
   ...
   where i.issuer_entity_id = p_issuer_id
    ...
    select pos, jsonb_agg(inv_row) as inv_array
      from b2cl_rows
     group by pos
  )
  ...
  -- ══════════════════════════════════════════════════════════════════════
  -- B2CS (unchanged — this section already aggregated correctly)
  -- ══════════════════════════════════════════════════════════════════════
  with b2cs_src as (
    select
      case
